...
class Searcher:
    ...
    def __init__(self):
        self.path = os.getcwd() + '/'
        self.tfidf_lemmas_folder = self.path.replace("demo", "task4") + 'tfidf_lemmas'
        self.tfidf_lemmas_path = self.tfidf_lemmas_folder + '/'
        self.tfid_file_name_prefix = 'tf_idf_'
        self.lemma_tokens_file_name = self.path.replace("demo", "task2") + 'lemma_tokens.txt'
        self.inverted_index_file_name = self.path.replace("demo", "task3") + 'inverted_index.json'
        self.index_file_name = self.path.replace("demo", "task1") + 'index.txt'
    ...
    def get_index(self):
        index = {}
        with open(self.index_file_name, encoding='utf-8') as file:
            lines = file.readlines()
            for line in lines:
                num, link = line.rstrip('\n').split(' ')
                index[num] = link
        return index
    ...
    def get_links_by_result(self, results):
        if results is not None and results:
            return {self.index[key]: value for key, value in results.items()}
        else:
            return None
